...
func RunMigrations(ctx context.Context, db *pgxpool.Pool) error {
	...
	if _, err := db.Exec(ctx, `CREATE TABLE IF NOT EXISTS schema_migrations (
     version VARCHAR PRIMARY KEY,applied_at TIMESTAMP NOT NULL DEFAULT NOW())`); err != nil {
  ...
  //scan works like rows
		...
		//permanently save both schema chnages and migration occurs
		if err := transactions.Commit(ctx); err != nil {
			return fmt.Errorf("failed to commit transaction for migrati on %s: %w", filename, err)
		}
